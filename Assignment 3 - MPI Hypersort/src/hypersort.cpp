...
void getLoad(string in_file, Info &ProcInfo) {
    ...
    // scatter the array
    MPI_Scatter(ProcInfo.input, ProcInfo.proc_load_size, MPI_LONG,
                ProcInfo.proc_array, ProcInfo.proc_load_size, MPI_LONG, 0,
                MPI_COMM_WORLD);
}
...
int main(int argc, char *argv[]) {
    ...
    while (runs < ProcInfo.comm_size) {
        ...
        MPI_Comm_split(current_group, ProcInfo.proc_Id < group_size / 2,
                       ProcInfo.proc_Id, &current_group);
        ...
        // split array
        int64_t low_size =
            upper_bound(ProcInfo.proc_array,
                        ProcInfo.proc_array + ProcInfo.proc_load_size, pivot) -
            ProcInfo.proc_array;
        ...
        if (ProcInfo.proc_Id >= group_size / 2) {
            ...
            int64_t temp;
            MPI_Recv(&temp, 1, MPI_LONG, partner_Id, 0, current_group,
                     MPI_STATUS_IGNORE);
            MPI_Recv(ProcInfo.low, temp, MPI_LONG, partner_Id, 0, current_group,
                     MPI_STATUS_IGNORE);
            MPI_Send(&low_size, 1, MPI_LONG, partner_Id, 0, current_group);
            MPI_Send(ProcInfo.proc_array, low_size, MPI_LONG, partner_Id, 0,
                     current_group);
            copy(ProcInfo.proc_array + low_size,
                 ProcInfo.proc_array + low_size + high_size, ProcInfo.high);
            low_size = temp;
        } else {
            // swap high for low region
            MPI_Send(&high_size, 1, MPI_LONG, partner_Id, 0, current_group);
            MPI_Send(ProcInfo.proc_array + low_size, high_size, MPI_LONG,
                     partner_Id, 0, current_group);
            MPI_Recv(&high_size, 1, MPI_LONG, partner_Id, 0, current_group,
                     MPI_STATUS_IGNORE);
            MPI_Recv(ProcInfo.high, high_size, MPI_LONG, partner_Id, 0,
                     current_group, MPI_STATUS_IGNORE);
            copy(ProcInfo.proc_array, ProcInfo.proc_array + low_size,
                 ProcInfo.low);
        }
        ProcInfo.proc_load_size = low_size + high_size;
        runs *= 2;
        merge(ProcInfo.low, ProcInfo.low + low_size, ProcInfo.high,
              ProcInfo.high + high_size, ProcInfo.proc_array);
    }
    ...
    if (ProcInfo.proc_Id != 0) {
        ...
        MPI_Send(ProcInfo.proc_array, ProcInfo.proc_load_size, MPI_LONG, 0, 0,
                 MPI_COMM_WORLD);
    } else {
        // gather from all procs
        copy(ProcInfo.proc_array, ProcInfo.proc_array + ProcInfo.proc_load_size,
             ProcInfo.input);
        int64_t final = ProcInfo.proc_load_size;
        for (int64_t i = 1; i < ProcInfo.comm_size; i++) {
            int64_t chunk;
            MPI_Recv(&chunk, 1, MPI_LONG, i, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
            MPI_Recv(ProcInfo.input + final, chunk, MPI_LONG, i, 0,
