...
population::population(string input_file, int pop_size, 
	int num_elites, double cross_prob, double mut_prob) {
...
void weed(string &C1, string &C2, string &swap1, string &swap2, int mem_size, int strt, int end) {
...
void population::mutate(string &chrm) {
	...
	if (random < mut_prob) { //mutate
		int r1 = (int)(getRandom(member_size)*member_size / (member_size + 1));
		int r2 = (int)(getRandom(member_size)*member_size/(member_size + 1));
		while (r2 == r1) {
			r2 = (int)(getRandom(member_size)*member_size / (member_size + 1));
		}
		char temp = chrm[r1];
		chrm[r1] = chrm[r2];
		chrm[r2] = temp;
